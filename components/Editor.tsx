...
import { getEditorProps, getExtensions, proseFont } from '@/helpers/tiptap.config'
...
export default function EditorComponent({ id, title, createdAt, updatedAt, content }: Page): JSX.Element {
  const { data, isLoading, error } = useSWR<Page[], Error>(`${apiBaseUrl}/page`)
    ...
    <div className="pb-4 border-gray-200 dark:border-gray-700 border-b-2">
      ...
      <div className="flex gap-2 flex-row">
        ...
        <div className="text-gray-600 dark:text-gray-400">
          ...
          <p>{!isNewPage ? `Last saved ${dayjs().from(dayjs(page.lastSaved), true)} ago` : `Unsaved draft...`}</p>
        ...
        <div className="flex gap-2 h-10 grow flex-row place-content-end">
          ...
          <span title={isNewPage ? "Autosave disabled on new pages. Manually save your work" : page.unsavedChanges ? "Unsaved changes..." : "Saved"} className="mr-4">
            <svg className={`py-3 px-2 ${isNewPage ? 'fill-red-500' : !page.unsavedChanges ? 'fill-primary-700' : 'fill-amber-500 animate-pulse'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512z" /></svg>
          </span>
